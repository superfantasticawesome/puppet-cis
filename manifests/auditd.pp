...
class cis::auditd {
  ...
  $auditd_rules.each |String $description, Hash $rule| {
    if 'x64' in "${description}" and ! '64' in "${::architecture}" {
      notice("OS architecture '${::architecture}' doesn't support this rule")
    } else {
